...
class FacebookConversionService
{
    ...
    {
        if (!$eventId) {
            $eventId = 'pageview_' . time();
        }
        
        $this->sendToFacebook("PageView", [], $eventId);
    }

    public function sendInitiateCheckout($combinedOrder, $eventId = null)
    {
        $contentIds = [];
        $value = 0;
        ...
        if (!$eventId) {
            $eventId = 'checkout_' . $combinedOrder->id . '_' . time();
        }
        
        $this->sendToFacebook("InitiateCheckout", [
            "currency" => Currency::findOrFail(get_setting('system_default_currency'))->code,
            "value" => $value,
            "content_ids" => $contentIds,
            "content_type" => "product"
    ...
    {
        if (!$eventId) {
            $eventId = 'registration_' . ($userId ?? time()) . '_' . time();
        }
        
        $this->sendToFacebook("CompleteRegistration", [
            "currency" => Currency::findOrFail(get_setting('system_default_currency'))->code,
        ], $eventId);
    }
}